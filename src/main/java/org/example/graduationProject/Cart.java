...
public class Cart {
     ...
     * (всегда вызывается совместно с операцией new). Конструктор нужен для автоматической
     * инициализации переменных, т.е. он инициализирует объект непосредственно во время создания.
     * Имя конструктора совпадает с именем класса, включая регистр, а по синтаксису конструктор
     * похож на метод без возвращаемого значения void.*/
    public Cart(Shop shop) {
        this.shop = shop;
    }
    /** Методы представляют собой подпрограммы, которые манипулируют данными, определенными
     * в классе, а во многих случаях они предоставляют доступ к этим данным. Как правило,
     * другие части программы взаимодействуют с классом посредством его методов. Метод состоит
     * из одного или нескольких операторов. В грамотно написанной программе на Java каждый метод
     * выполняет только одну функцию. У каждого метода имеется свое имя, по которому он вызывается.*/
    ...
    // Метод по пересчитывании суммы покупки
    ...
    // Метод по добавлению в корзину продукта
    private void addToCart(Product product) {
        ...
        // Изменяю количество в корзине +1
    ...
    // Метод по удалению продукта по идентификатору
    ...
    void printCartItems() {
        ...
        System.out.println("Сейчас у  меня в корзине:");
        System.out.format(format, "ID", "Название", "Цена, р.", "Количество в корзине, штук");
    ...
    // Метод установления общей суммы
